...
void Puffs::addInstances(Array<PuffShader::InstanceData>& instances) const {
    ...
    Float3 basePos = this->pos + Float3{0.3f, 0, 1.f} * (offset * 0.7f);
    ...
    for (u32 i = 0; i < numPuffs; i++) {
        ...
        Float3 pos = basePos + Quaternion::fromAxisAngle(axis, Pi * ((i + 0.5f) * side / numPuffs + 0.5f)) *
        ...
        inst.colorAlpha = {this->time, alpha * 0.7f};
        side *= -1.f;
    }
}
